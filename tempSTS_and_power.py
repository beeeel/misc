from dataReading import tempReaderSTS, tempReader, powerReader
import time
import os
...
tempSTS = tempReaderSTS()

starttime = time.monotonic()
tnow = 0
data = str(tnow) + ', ' + powrR.read() + tempR.read() + tempSTS.read()
...
for i in range(nTSTS):
    header = header + ', STS temperature ' + str(i+1) + ' (deg C)'
...
while tnow-starttime < maxTime:
    ...
    tnow = time.monotonic()
    data = str(tnow-starttime) + ', ' + powrR.read() + tempR.read() + tempSTS.read()
